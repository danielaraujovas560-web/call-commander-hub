...
function formatDias(dias: string): string {
  return dias
    .split("&")
    .map((d) => DIA_FULL[d.trim().toLowerCase()] ?? d)
    .join(", ");
}
...
function Page() {
  ...
  const delMut = useMutation({
    mutationFn: (regra_identifier: string) =>
      delFn({ data: { regra_identifier, tenant_id: tenantId } }),
    onSuccess: () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Clock className="h-6 w-6" /> Horário de Atendimento Personalizado
          </h1>
          <p className="text-sm text-muted-foreground">
            Regras de horário e destinos dentro/fora do expediente.
          </p>
        </div>
            ...
            {regras.map((r) => (
              <TableRow key={r.regra_identifier}>
                ...
                <TableCell className="font-mono text-xs">
                  {r.hora_inicial} → {r.hora_final}
                </TableCell>
                <TableCell>
                  <Badge variant="default">{r.acao_dentro}</Badge>{" "}
                  <span className="text-xs ml-1">
                    {renderDestinoLabel(destinos, r.acao_dentro, r.destino_dentro)}
                  </span>
                </TableCell>
                <TableCell>
                  <Badge variant="secondary">{r.acao_fora}</Badge>{" "}
                  <span className="text-xs ml-1">
                    {renderDestinoLabel(destinos, r.acao_fora, r.destino_fora)}
                  </span>
                </TableCell>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => delMut.mutate(r.regra_identifier)}>
                            Remover
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
...
function RegraFormDialog({
  ...
  const initialFora = (): DestinoValue =>
    regra
      ? parseDestinoFromBackend(regra.acao_fora, regra.destino_fora)
      : { ...emptyDestino, tipo: "INTERNO", destino: "desligar" };
  ...
  const canSubmit =
    !!nome && dias.length > 0 && !isDestinoIncomplete(dentro) && !isDestinoIncomplete(fora);
